...
export default function DataImportScreen() {
  const { goBack, funds, setFunds, refreshData } = useContext(NavigationContext);
      ...
      try {
        ...
        if (file.name.endsWith('.json')) {
          ...
          for (let i = startIndex; i < lines.length; i++) {
            ...
            if (parts.length >= 1) {
              let code = parts[0].replace(/[^0-9]/g, '');
              if (code) {
                code = code.padStart(6, '0');
                const amount = parseFloat(parts[1]) || 0;
                const holdingReturn = parseFloat(parts[2]) || 0;
                const group = parts[3] || '未分组';
          ...
          if (newPreviewFunds.length > 0) {
            ...
            setShowPreview(true);
            newPreviewFunds.forEach(pf => fetchFundName(pf.code, pf.id));
          } else {
            alert('未找到有效的基金数据');
          }
          setIsImporting(false);
  ...
  const handleConfirmImport = () => {
    const newFunds = previewFunds.map(pf => {
      const totalReturnRate = pf.amount > 0 ? (pf.holdingReturn / (pf.amount - pf.holdingReturn)) * 100 : 0;
      return {
        id: Date.now().toString() + Math.random().toString(36).substring(7),
        code: pf.code,
        name: pf.name !== '加载中...' && pf.name !== '未知基金' && pf.name !== '查询失败' ? pf.name : pf.code,
        amount: pf.amount,
        returnRate: 0,
  ...
  const handlePreviewChange = (id: string, field: keyof PreviewFund, value: string | number) => {
    setPreviewFunds(prev => prev.map(f => {
      if (f.id === id) {
    ...
    return (
      <div className="flex-1 flex flex-col bg-slate-50 dark:bg-background-dark h-full">
        <div className="sticky top-0 z-10 flex items-center justify-between px-4 py-3 bg-white dark:bg-[#192633] border-b border-slate-100 dark:border-slate-800">
          <button onClick={() => setShowPreview(false)} className="flex size-10 items-center justify-center rounded-full hover:bg-slate-100 dark:hover:bg-slate-800 transition-colors">
            <span className="material-symbols-outlined text-[24px]">arrow_back</span>
          </button>
          <h2 className="text-lg font-bold leading-tight tracking-tight text-center flex-1">确认导入数据</h2>
              ...
              <table className="w-full text-sm text-left">
                ...
                <tbody className="divide-y divide-slate-100 dark:divide-slate-800/50">
                  {previewFunds.map((fund) => (
                    <tr key={fund.id} className="hover:bg-slate-50 dark:hover:bg-slate-800/30 transition-colors">
                      <td className="px-4 py-3">
                        <div className="flex flex-col gap-1">
                          <input 
                            type="text" 
                            value={fund.code}
                            onChange={(e) => handlePreviewChange(fund.id, 'code', e.target.value)}
                            ...
                            maxLength={6}
                          />
                          <span className={`text-xs ${fund.isLoadingName ? 'text-slate-400 animate-pulse' : 'text-slate-500 dark:text-slate-400'}`}>
                            {fund.name}
                          </span>
                        </div>
                      </td>
                      <td className="px-4 py-3">
                        <input 
                          type="number" 
                          value={fund.amount}
                          onChange={(e) => handlePreviewChange(fund.id, 'amount', parseFloat(e.target.value) || 0)}
                      ...
                      <td className="px-4 py-3">
                        <input 
                          ...
                          onChange={(e) => handlePreviewChange(fund.id, 'holdingReturn', parseFloat(e.target.value) || 0)}
                          className="w-24 bg-transparent border-b border-slate-200 dark:border-slate-700 focus:border-primary outline-none text-slate-900 dark:text-white"
                        />
                      </td>
                      <td className="px-4 py-3">
                        <input 
                          type="text" 
                          value={fund.group}
                          onChange={(e) => handlePreviewChange(fund.id, 'group', e.target.value)}
                          className="w-24 bg-transparent border-b border-slate-200 dark:border-slate-700 focus:border-primary outline-none text-slate-900 dark:text-white"
                        />
                      </td>
                      <td className="px-4 py-3 text-center">
                        <button onClick={() => handleDeletePreview(fund.id)} className="text-red-500 hover:text-red-600 p-1 rounded-full hover:bg-red-50 dark:hover:bg-red-900/20 transition-colors">
        ...
        <div className="p-4 bg-white dark:bg-[#192633] border-t border-slate-100 dark:border-slate-800 pb-safe">
          <button 
            ...
            disabled={previewFunds.length === 0 || previewFunds.some(f => f.isLoadingName)}
          ...
          >
            ...
            {isImporting ? (
              <div className="flex flex-col items-center justify-center">
                ...
                <p className="text-sm font-medium text-slate-700 dark:text-slate-300">正在导入...</p>
              </div>
            ) : (
              <>
                ...
                <p className="text-[10px] text-slate-400 mt-2">CSV格式建议: 代码, 金额, 持有收益, 分组</p>
              </>
            )}
          </div>
        )}
            ...
            <div className="space-y-4">
              <div className="border border-slate-200 dark:border-slate-700 rounded-lg overflow-hidden">
                <button className="w-full flex items-center justify-between p-3 bg-slate-50 dark:bg-slate-800/50 hover:bg-slate-100 dark:hover:bg-slate-800 transition-colors">
                  <div className="flex items-center gap-3">
                    ...
                    <span className="text-sm font-medium text-slate-900 dark:text-white">支付宝账单导入指南</span>
                  </div>
                  <span className="material-symbols-outlined text-slate-400">expand_more</span>
                </button>
                <div className="p-4 bg-white dark:bg-[#192633] text-sm text-slate-600 dark:text-slate-400 space-y-2">
                  <p className="font-bold text-slate-800 dark:text-slate-200">对接支付宝需要您执行以下操作：</p>
                  <ol className="list-decimal pl-5 space-y-1">
                    <li>打开支付宝 App，进入 <strong>我的</strong> {'->'} <strong>账单</strong>。</li>
                    <li>点击右上角的 <strong>...</strong>，选择 <strong>开具交易流水证明</strong>。</li>
                    <li>选择 <strong>用于个人对账</strong>，选择时间范围。</li>
                    <li>输入您的电子邮箱地址，点击发送。</li>
                    <li>在电脑或手机上打开邮箱，下载并解压包含账单的 ZIP 文件。</li>
                    <li>回到本应用，切换到 <strong>本地文件</strong> 导入，选择解压出的 CSV 文件即可。</li>
                  </ol>
                  <div className="mt-3 p-2 bg-blue-50 dark:bg-blue-900/20 rounded text-xs text-blue-600 dark:text-blue-400">
                    注：目前支付宝不提供直接的 API 授权读取持仓，通过账单流水导入是最安全可靠的方式。
                  </div>
                </div>
              </div>
              ...
              <button className="w-full flex items-center justify-between p-3 rounded-lg border border-slate-200 dark:border-slate-700 hover:bg-slate-50 dark:hover:bg-slate-800 transition-colors">
                <div className="flex items-center gap-3">
                  ...
                  <span className="text-sm font-medium text-slate-900 dark:text-white">天天基金 (开发中)</span>
                </div>
