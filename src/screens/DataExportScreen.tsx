...
export default function DataExportScreen() {
  ...
  const handleExport = () => {
    if (exportFormat === "json") {
      ...
      if (exportHoldings) {
        csvContent +=
          "基金代码,基金名称,持有金额,持有收益,持有收益率,今日收益率,分组\n";
        funds.forEach((fund) => {
          const principal = fund.amount / (1 + fund.totalReturnRate / 100);
          const holdingReturn = fund.amount - principal;

          const row = [
            fund.code,
            fund.name,
            fund.amount.toFixed(2),
            holdingReturn.toFixed(2),
            fund.totalReturnRate.toFixed(2),
      ...
      linkElement.setAttribute(
        "download",
        `fund_data_${new Date().toISOString().split("T")[0]}.csv`,
      );
      linkElement.click();
    }
        ...
        <div className="space-y-3">
          ...
          <div className="bg-white dark:bg-[#192633] rounded-xl border border-slate-100 dark:border-slate-800/50 overflow-hidden">
            <label className="flex items-center p-4 border-b border-slate-100 dark:border-slate-800 cursor-pointer hover:bg-slate-50 dark:hover:bg-slate-800/50 transition-colors">
              <input
                type="checkbox"
                checked={exportHoldings}
                onChange={(e) => setExportHoldings(e.target.checked)}
